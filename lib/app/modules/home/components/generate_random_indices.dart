...
class RandomIndexGenerator {
  ...
  List<int> generateRandomIndices(int length, int minDistance) {
    ...
    if (length > currentLength) {
      ...
      while (randomIndices.length < maxSecondaryListViews && attempts < maxAttempts) {
        int randomIndex = random.nextInt(currentLength); // Generate random index within the new length

        // Ensure that the new index has a minimum distance from all other indices
        bool isValid = true;
        for (int index in randomIndices) {
          if ((randomIndex - index).abs() < (minDistance??0)) {
            isValid = false;
            break;
          }
        }
